...
<div class="content-section">
    ...
    <?php if (isset($error)): ?>
        ...
        <script>
            window.addEventListener('DOMContentLoaded', () => {
                showMessageBar("Unable to add category. Please see the error below.", "error");
            });
        </script>
    <?php endif; ?>
    ...
    <div class="form-container modern-form">
        <div class="form-header">
            <h3>Add New Pet Category</h3>
            <p>Define new species or breeds for product categorization.</p>
        </div>
        <form method="POST" action="" id="petTypeForm" onsubmit="return confirmAddition(event)">
            ...
            <div class="form-actions-row" style="margin-top: 20px;">
                <button type="button" class="btn-clear visible-btn" onclick="clearForm('petTypeForm')"><i class="fas fa-undo"></i> Clear Values</button>
                <button type="submit" name="add_pet_type" class="btn-primary">
            ...
            <table style="background: #fff; width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background: linear-gradient(135deg, #e0f2fe 0%, #dbeafe 100%); border-bottom: 2px solid #bfdbfe;">
                        ...
                        <th style="width: 85%; padding: 16px; text-align: left; font-weight: 700; color: #1e40af; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 1px;">Pet Type Name</th>
                    </tr>
                </thead>
                <tbody>
...
function confirmAddition(e) {
    ...
    showConfirmation(
        '🐾 Add Pet Type',
        'Are you sure you want to add this new pet category?',
        '🐾',
        () => {
            const submitBtn = document.createElement('input');
            submitBtn.type = 'hidden';
            submitBtn.name = 'add_pet_type';
            submitBtn.value = '1';
